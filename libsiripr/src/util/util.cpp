...
namespace siripr {
	...
	bool Utils::imwrite(const std::string & path, const std::string &name, const cv::Mat img)
	{
		static int id = 0;
		...
		time(&timep);
		p = localtime(&timep);
		sprintf(str, "%02d%02d_%02d_", p->tm_min, p->tm_sec, id++);
		return Utils::imwrite(path + str + name, img);
	}
