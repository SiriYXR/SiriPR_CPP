...
namespace siripr {
	...
	int CCharsSegment::charsSegmentUsingOSTU(Mat input, vector<Mat>& resultVec, vector<Mat>& grayChars, Color color) {
		...
		if (m_debug)
			Utils::imwrite(kTempImgOutputPath, "plate_clearLiuDing.jpg", img_threshold);
		...
		if (m_debug) {
			rectangle(img_threshold, chineseRect, Scalar(255));
			Utils::imwrite(kTempImgOutputPath, "plate_chineseRect.jpg", img_threshold);
		}
		...
		for (size_t i = 0; i < newSortedRect.size(); i++) {
			...
			if (i == 0) {
				...
				if (m_debug)
					Utils::imwrite(kTempImgOutputPath, "plate_newChineseRoi.jpg", newChineseRoi);
			}
			else {
				switch (plateType) {
				case BLUE:   threshold(auxRoi, newRoi, 0, 255, CV_THRESH_BINARY + CV_THRESH_OTSU); break;
				...
				if (m_debug)
					Utils::imwrite(kTempImgOutputPath, "plate_grayChar.jpg", grayChar);
			}

			if (m_debug)
				Utils::imwrite(kTempImgOutputPath, "plate_newRoi.jpg", newRoi);
