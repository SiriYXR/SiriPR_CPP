...
namespace siripr {
	...
	//Coding by siri
	Mat colorMatch2(const Mat &src, Mat &match, const Color r,
		const bool adaptive_minsv) {

		// if use adaptive_minsv
		// min value of s and v is adaptive to h
		const float max_sv = 255;
		const float min_sv = 95; //95;

								 // H range of blue 

		const float minabs_s1 = 255 * 0.45;
		const float minabs_v1 = 255 * 0.25;

		const int min_blue = 100;  // 200
		const int max_blue = 140;  // 280
		...
		for (i = 0; i < nRows; ++i) {
			for (j = 0; j < nCols; ++j) {
				int H = int(hsvSplit[0].at<uchar>(i, j));      // 0-180
				int S = int(hsvSplit[1].at<uchar>(i, j));  // 0-255
				int V = int(hsvSplit[2].at<uchar>(i, j));  // 0-255
				...
				if (colorMatched == true) {
					hsvSplit[0].at<uchar>(i, j) = 0;
					hsvSplit[1].at<uchar>(i, j) = 0;
					hsvSplit[2].at<uchar>(i, j) = 255;
				}
				else {
					hsvSplit[0].at<uchar>(i, j) = 0;
					hsvSplit[1].at<uchar>(i, j) = 0;
					hsvSplit[2].at<uchar>(i, j) = 0;
				}
			}
		}
	...
	bool colorSVMatch(const int S, const int V) {
		const float maxSV = 255;
		const float minSV[][2] = { { 255 * 0.35,255 * 0.35 },{ 255 * 0.45,255 * 0.25 },{ 255 * 0.42,255 * 0.31 },{ 255 * 0.31,255 * 0.42 } };
		const int minSize = 3;
		bool isOK = false;

		for (int i = 0; i < minSize; i++) {
			if ((S > minSV[i][0] - 1 && S < maxSV) && (V > minSV[i][1] - 1 && V < maxSV)) {
				isOK = true;
				break;
			}
		}
...
#pragma omp parallel for
	...
	/*
	根据预定最大尺寸将原图片保持原有比例缩放
	*/
	Mat uniformResize(const Mat &result, float &scale) {
		const int RESULTWIDTH = kShowWindowWidth;   // 1000
		const int RESULTHEIGHT = kShowWindowHeight;  // 800
	...
	Mat showResult(const Mat &result, int img_index) {
		namedWindow("SiriPR", CV_WINDOW_AUTOSIZE);
		...
		if (1) {
			imshow("SiriPR", img_window);
			waitKey(0);
			destroyWindow("SiriPR");
		}
