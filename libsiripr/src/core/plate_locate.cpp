#include "siripr/core/plate_locate.h"
...
namespace siripr {
	...
	int CPlateLocate::colorSearch(const Mat &src, const Color r, Mat &out,
		...
		colorMatch2(src, match_grey, r, false);

		if (m_debug) 
			Utils::imwrite(kTempImgOutputPath, "color_colorMatch2.jpg", match_grey);
		...
		if (m_debug)
			Utils::imwrite(kTempImgOutputPath, "color_OTSU_Binary.jpg", src_threshold);
		...
		if (m_debug)
			Utils::imwrite(kTempImgOutputPath, "color_morphologyEx.jpg", src_threshold);
	...
	int CPlateLocate::sobelSecSearchPart(Mat &bound, Point2f refpoint,
		...
		if (bFindLeftRightBound(tempBoundThread, posLeft, posRight)) {
			...
			//utils::imwrite(kTempImgOutputPath, "resources/image/tmp/repaireimg1.jpg", bound_threshold);
			...
			//utils::imwrite(kTempImgOutputPath, "resources/image/tmp/repaireimg2.jpg", bound_threshold);
		}
	...
	int CPlateLocate::sobelOper(const Mat &in, Mat &out, int blurSize, int morphW,
		...
		if(m_debug)
			utils::imwrite(kTempImgOutputPath, "sobel_GaussianBlur.jpg", mat_blur);
