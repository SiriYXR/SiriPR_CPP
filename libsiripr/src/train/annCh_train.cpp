...
#include "siripr/train/annCh_train.h"
...
#include "siripr/train/create_data.h"
...
namespace siripr {
	...
	void AnnChTrain::train()
	{
		...
		bool useTLFN = false;
		if (!useTLFN) {
			fprintf(stdout, ">> Use one-layers neural networks,\n");
			fprintf(stdout, ">> Input_neurons: %d \n", input_number);
			fprintf(stdout, ">> Hidden_neurons: %d \n", hidden_number);
			fprintf(stdout, ">> Output_neurons: %d \n", output_number);
